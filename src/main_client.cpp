...
// Для удобства зададим какой-то дефолтный пароль (в реальном приложении храните/спрашивайте безопасно)
...
bool read_message(tcp::socket& socket, Message& out_msg, boost::system::error_code& ec) {
    ...
    uint32_t length = static_cast<uint32_t>(length_buf[0]) 
        | (static_cast<uint32_t>(length_buf[1]) << 8)
        | (static_cast<uint32_t>(length_buf[2]) << 16)
        | (static_cast<uint32_t>(length_buf[3]) << 24);
    
    if (length > 10*1024*1024) { // Защита от больших сообщений
...
void background_reader(tcp::socket& socket) {
    ...
    while (true) {
        ...
        std::cout << "[Got message] From: " << msg.sender 
                  << " | Text: " << msg.text 
...
int main(int argc, char* argv[]) {
    ...
    // 1) Отправляем сообщение регистрации (ClientRegistration)
    {
        ...
        std::cout << "[Client] Server response: " << resp.text << ", filename=" << resp.filename << std::endl;
        if (resp.text == "Error") {
